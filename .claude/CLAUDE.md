...
Offline, privacy-first voice dictation for **Windows** and **Android**. Speech is transcribed on-device (with an optional Groq cloud path on Windows), cleaned through a deterministic pipeline, and inserted into the focused text field.
...
- **Offline default**: whisper.cpp (Windows), sherpa-onnx (Android target).
