...
// --- microphone picker -------------------------------------------------------
// Devices, level meters and verification all come from the Rust core (cpal), not
// from getUserMedia: the mic is owned by the native side on Linux, so the picker
// asks for a live preview stream per device and tears them all down on close.
let micLevelWired = false;
const micMeters = new Map();

function wireMicLevels() {
  if (micLevelWired) return;
  micLevelWired = true;
  cv.on('mic:levels', (data) => {
    if (!data) return;
    for (const meter of micMeters.get(data.deviceId) || []) paintMeter(meter, data.level);
  });
  cv.on('mic:dead', (data) => {
    if (!data) return;
    for (const meter of micMeters.get(data.deviceId) || []) {
      const row = meter.closest('.mic-option');
      if (!row) continue;
      row.classList.add('unavailable');
      const sub = row.querySelector('.mic-sub');
      if (sub) {
...
function paintMeter(meterEl, level) {
  const bars = meterEl.children;
  // At least one bar stays lit while the mic is alive, so a silent-but-working
  // mic still reads as connected.
  const lit = Math.max(1, Math.round(Math.min(1, level || 0) * bars.length));
...
function listMicrophones() {
  return cv.invoke('mic:list');
}

function closeMicrophoneModal() {
  micMeters.clear();
  cv.invoke('mic:previewStop').catch(() => {});
  modalRoot.innerHTML = '';
}
...
function renderMicrophoneOptions(container, devices, settings) {
  ...
  const fallback = (devices.find((d) => d.default) || devices[0] || {}).deviceId || '';
  const options = [
    { id: '', meterId: fallback, label: 'Auto-detect', sub: 'From your sound settings', selected: !selected },
    ...devices.map((device) => ({
      id: device.deviceId,
  ...
  for (const option of options) {
    ...
    if (option.meterId) {
      if (!micMeters.has(option.meterId)) micMeters.set(option.meterId, []);
      micMeters.get(option.meterId).push(meter);
    }
    btn.addEventListener('click', async () => {
      try {
        await cv.invoke('mic:verify', option.id);
        ...
        sub.textContent = 'This microphone did not start. Check it in your sound settings.';
        sub.classList.add('mic-err');
        sub.style.display = '';
      }
    });
  ...
  cv.invoke('mic:previewStart', options.map((o) => o.meterId).filter(Boolean)).catch(() => {});
}

async function refreshMicModal(list) {
  if (!modalRoot.querySelector('.mic-backdrop')) return;
  try {
    const [devices, settings] = await Promise.all([listMicrophones(), cv.invoke('db:getSettings')]);
    if (!modalRoot.querySelector('.mic-backdrop')) return;
    if (!devices.length) {
      micMeters.clear();
      renderMicNotReady(list, 'No microphone was detected. Plug one in, or pick a different input in your sound settings.');
      return;
    }
    renderMicrophoneOptions(list, devices, settings);
  } catch {
    micMeters.clear();
    renderMicNotReady(list, 'Your microphone is not connected or could not start. Reconnect it, or choose another input in your sound settings.');
  }
}
...
function openMicrophoneModal() {
  ...
  modalRoot.appendChild(backdrop);
  // The Rust mic watcher is the devicechange equivalent: it fires whenever a
  // device appears or disappears, so the list stays live while the picker is up.
  cv.on('mic:status', () => refreshMicModal(list));
  refreshMicModal(list);
}
  ...
  const backdrop = el('div', { class: 'modal-backdrop mic-backdrop micready-backdrop', onclick: (e) => { if (e.target === backdrop) close(); } }, [
    el('div', { class: 'mic-modal mic-modal-slim' }, [
      ...
      el('div', { class: 'mic-notready' }, [
        el('div', { class: 'mic-notready-icon', html: ICON.mic }),
        el('div', { class: 'mic-notready-sub', text: 'Your microphone is not connected or could not start. Reconnect it, or choose another input in your sound settings.' }),
        el('button', {
          class: 'btn mic-sound-btn',
          type: 'button',
          onclick: () => cv.invoke('app:openSoundSettings'),
